<template>
  ...
  <template #title>
    ...
    <v-container>
            ...
            <v-row>
              <v-col cols="12" md="8">
                <v-text-field
                  ...
                  :error-messages="getErrors('company.registration', $v.company.registration)"
          ...
          <template #button>
      <v-btn color="blue darken-1" :disabled="$v.$invalid" text @click="saveCompany"> Save </v-btn>
    </template>
...
<script>
import { required, maxLength, minLength, alphaNum, numeric, email } from 'vuelidate/lib/validators';
...
export default {
  ...
  validations() {
    return {
      company: {
        ...
        registration: {required, minLength: minLength(5), maxLength: maxLength(50)},
  ...
  methods: {
    ...
    saveCompany() {
      ...
      if (this.isEdit) {
        this.$store.dispatch("company/update_company", this.company);
      } else {
        this.$store.dispatch("company/create_company", this.company);
      }
      this.closeDialog();
    },
  },
